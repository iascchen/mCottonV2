...
Meteor.publish('geo_near_events', function (location, distance, timeAfter, limit) {
    //if (!this.userId) {
    //    return this.ready();
    //}
    ...
    return C_MessageDatas.aggregate([{
        $geoNear: { near: location, distance: distance, query: { createAt: { $gt: timeAfter } } }
    },
        {
            $group: { deviceId: "$deviceId", location: { $last: "payload.location" } }
        }]);
});

FindFromPublication.publish('msg_datas_find', function (query, limit) {
    return C_MessageDatas.find(query,
        { sort: { createdAt: -1 }, limit: Math.min(limit, MAX_LIMIT) });
});